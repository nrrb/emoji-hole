...
const EmojiTypes = {
  ...
  PLAYER: {
    ...
    type: "player",
    baseSizeValue: 1,
    sizeValue: 1  // Added explicit sizeValue for the player
  },
...
const EmojiUtils = {
  ...
  getConsumableEmojis(playerSize) {
    ...
    Object.values(EmojiTypes).forEach(group => {
      if (group.emojis && group.minSizeToConsume) {
        if (playerSize >= group.minSizeToConsume) {
  ...
  getSpawnableEmojis(playerSize) {
    ...
    Object.values(EmojiTypes).forEach(group => {
      if (group.emojis && group.minSizeToConsume) {
        // For a balanced gameplay, spawn items that are relevant to the current player size
        // Include some smaller items, mostly appropriate items, and a few challenging larger items
        const lowerBound = playerSize * 0.5;
        const upperBound = playerSize * 1.5;
        
        group.emojis.forEach(emoji => {
          if (emoji.sizeValue && emoji.sizeValue >= lowerBound && emoji.sizeValue <= upperBound) {
            spawnable.push(emoji);
          }
        });
      }
    });
